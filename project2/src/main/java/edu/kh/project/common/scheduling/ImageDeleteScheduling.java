...
//				문자열, substring(인덱스) : 지정된 인덱스 부터 끝까지 잘라서 반환
				...
				String fileName
//				= file.toString().substring( file.toString().lastIndexOf("\\") + 1 ); // 파일명.jpg
				= file.getName();
				
				// dbList에서 fileName 과 일치하는 파일링이 없다면
				// == 서버에는 있는데 DB에 없는 파일
				if(dbList.indexOf(fileName) == -1) {
//					System.out.println(fileName + " 삭제");
					
					// trace, debug, info, wern, error, fatal
