...
// 1) 비밀번호 미작성 -> "비밀번호를 입력해주세요" 
// alert 출력 후 포커스 이동(내용도 같이 삭제)

// 2) 동의 체크가 되지 않은 경우 -> "탈퇴 동의하시면 체크를 눌러주세요."
// alert 출력 후 포커스 이동
...
if(memberDeleteForm != null){ // memberDeleteForm 요소가 존재할 때

    memberDeleteForm.addEventListener("submit", function(event){

        console.log(event);

        // 회원 탈퇴에 사용되는 input 요소 모두 얻어오기
...
// (ture : 업로드 된 이미지 있음, false : 기본 이미지)
let initCheck;

// 이미지가 업로드 되었거나 삭제되었음을 나타내는 변수
// (-1 : 초기값(취소), 0 : 프로필 삭제(x번 클릭), 1 : 새 이미지 업로드)
let deleteCheck = -1;
...
if (imageInput != null){
    ...
    imageInput.addEventListener("change", e => {
        ...
        if (e.target.files[0] != undefined){
            ...
            // FileReader
            // 웹 애플리케이션이 비동기적으로
            // 데이터를 읽기 위하여 읽을 파일을 가리키는 File 객체 
            // - 읽어들인 파일을 사용자 컴퓨터에 저장할 수 있다.
    
            reader.readAsDataURL(e.target.files[0]);
            // FileReader.readAsDataURL(파일 요소")
            // -> 지정된 파일을 읽기 시작함.
            ...
            reader.onload = event => {
                ...
                // event.target.result : 읽어진 파일 결과(실제 이미지 파일)
                event.target.result;
